/* assets.js */
/*
    The assets object is a central store house
    for all the games assets. These are pre-
    loaded prior to the game being intitiated.

    This is a memory efficient way to handle
    all the files that the program needs at
    rutime.

    The assets object has a `load()` method
    that accepts one argument: an array of
    filename strings, a list of all the files
    we want to load. When the load method  has
    commpleted it return a promise, in this case,
    a callback function initialises the game.
*/
